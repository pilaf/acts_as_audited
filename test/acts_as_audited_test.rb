...
class ActsAsAuditedTest < Test::Unit::TestCase
  should "include instance methods" do
    User.new.should be_kind_of(ActsAsAudited::InstanceMethods)
  end

  should "extend singleton methods" do
    User.should be_kind_of(ActsAsAudited::SingletonMethods)
  end
  ...
  context "on create" do
    ...
    should_change 'ActsAsAudited::Audit.count', :by => 1
    ...
    should "not save an audit if only specified on update and on destroy" do
      lambda { on_update_destroy = OnUpdateDestroy.create(:name => 'Bart') }.should_not change { ActsAsAudited::Audit.count }
    end
  end
  ...
  context "on update" do
    ...
    should "not save an audit if the record is not changed" do
      lambda { @user.save! }.should_not change { ActsAsAudited::Audit.count }
    end
    ...
    if ActiveRecord::VERSION::STRING >= '2.3'
      should "not save an audit if the value doesn't change after type casting" do
        ...
        lambda { @user.update_attribute :logins, '0' }.should_not change { ActsAsAudited::Audit.count }
        lambda { @user.update_attribute :activated, 1 }.should_not change { ActsAsAudited::Audit.count }
        lambda { @user.update_attribute :activated, '1' }.should_not change { ActsAsAudited::Audit.count }
      end
    end
    ...
    should "not save an audit if only specified on create and on destroy" do
      ...
      lambda { on_create_destroy.update_attributes :name => 'Changed' }.should_not change { ActsAsAudited::Audit.count }
    end
  end
  ...
  context "on destroy" do
    ...
    should "save an audit" do
      lambda { @user.destroy }.should change { ActsAsAudited::Audit.count }.by(1)
    ...
    should "not save an audit if only specified on create and on update" do
      ...
      lambda { on_create_update.destroy }.should_not change { ActsAsAudited::Audit.count }
    end
  end
  ...
  context "revision_at" do
    should "find the latest revision before the given time" do
      u = create_user
      ActsAsAudited::Audit.update(u.audits.first.id, :created_at => 1.hour.ago)
